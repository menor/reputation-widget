...
var ResmioRep = (function(window, undefined) {
  ...
  function initialize() {
    var params = getScriptData();
    getFeedbackScore(params, function(res) {
      renderBadge(params)
    })
  }

  function getFeedbackScore(options, cb) {
    // This needs to call the feedback score API endpoint instead
    var req = makeCORSRequest('https://api.resmio.com/v1/facility/' + options.id + '/availability?date__gte=2016-03-01')
  ...
  function renderBadge() {
    ...
    for (var i = 0; i < targets.length; i++) {
      ...
      var options = {
        ...
        language: location.getAttribute('data-resmio-reputation-language')
      };
  ...
  function getScriptData() {
    // Query the DOM for all script elements in the publisher page
    var scripts = document.getElementsByClassName('resmio-reputation-widget');
    var data = {};
    // Iterate through all the scripts until we find ours
    for (var i = 0; i < scripts.length; i++) {
      data.id = scripts[i].getAttribute('data-resmio-reputation-id');
      data.language = scripts[i].getAttribute('data-resmio-reputation-language');
      if (data) { console.log(i, data); return data; }
    }
    return null;
}
